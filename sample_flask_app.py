...
manager = Manager(app)
def search_similiar(word):
    baseurl = "https://api.datamuse.com/words"
    params = {}
    params['sl'] = word
    result_dict_list = json.loads(requests.get(baseurl,params = params).text)
    return random.choice(result_dict_list)['word']
# Routes
...
def datamuse(new_word):
    return '<h1>{}</h1>'.format(search_similiar(new_word))
...
def photo_titles(tag, num):
    ...
    FLICKR_KEY = "f6da062211b78e85b694c6a8799679cf" # TODO: fill in a flickr key
    ...
    num = len(flickr_data['photos']['photo'])
